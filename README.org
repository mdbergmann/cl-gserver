...
A =Router= is a facade over a set of actors. Routers are either created with a set of actors or actors can be added later.
Routers foward a =tell=, =ask= or =async-ask= to a 'routee' (one of the actors of a router) by passing all of the given parameters. The routee is chosen by applying a =strategy=.
By built-in default strategy a routee is chosen randomly.
The =strategy= can be configured when creating a router.
It is just a function that takes the number of routees and returns an index to be chosen for the next operation.
