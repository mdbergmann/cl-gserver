...
But let's just right into it. I'll explain more later.
...
=async-ask= combines both =ask= and =tell=. From =ask= is 'inherits' returning a result, even though it's a future result. But internally it's implemented using =tell= in that a temporary actor is spawned that waits until it receives the result from the actor where the message was sent to.
