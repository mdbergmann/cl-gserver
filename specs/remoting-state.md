...
- `src/remoting/transport.lisp` — `sento.remoting.transport` package (`:rtrans`), transport conditions (`transport-error`, `connection-refused-error`, `connection-timeout-error`, `connection-closed-error`, `send-failed-error`, `message-too-large-error`), abstract `transport` base class with `max-message-length` slot (default 2MB), protocol generics (`transport-start`, `transport-stop`, `transport-send`)
- `src/remoting/transport-tcp.lisp` — `sento.remoting.transport-tcp` package (`:rtrans-tcp`), `tcp-transport` implementation with 4-byte big-endian length-prefixed framing, TLS support, connection pooling (lazy reconnect), per-connection reader threads, accepted socket tracking for clean shutdown. `%read-frame` and `%write-frame` enforce `max-message-length`. Accept loop and reader threads use `wait-for-input` with 0.5s timeout polling for portable shutdown on Linux.
- `tests/remoting/transport-test.lisp` — 14 tests (start/stop lifecycle, stop closes connections, send envelope loopback, large message >64KB, connection reuse, connection failure reconnect, concurrent sends, connection-refused condition, TLS handshake failure propagation, send over TLS, default max-message-length, custom max-message-length, send rejects oversized, receive rejects oversized)
...
- `src/remoting/remote-ref.lisp` — `sento.remoting.remote-ref` package (`:rref`), conditions (`remote-actor-error`, `invalid-remote-uri-error`), `remote-actor-ref` class with internal sender actor for queued `tell`, direct send for `ask-s`/`ask`, correlation-id based response matching, URI parsing (`sento://host:port/path`)
...
### CI Integration (done)
- `.github/workflows/CI.yml` — added remoting test step; clones `pure-tls`, `cl-cancel`, `precise-time` from GitHub (not in Quicklisp); registers via ASDF source-registry
- `transport-tcp.lisp` — accept loop and reader threads use `wait-for-input` with 0.5s timeout for portable shutdown on SBCL/Linux (blocking socket I/O does not unblock on socket close)

**62 tests, 162 checks, all passing**
